  ...
  validate(control: AbstractControl): Promise<ValidationErrors | null> | Observable<ValidationErrors | null> {
    ...
    return this._usersPlaceholderService.getUsersPlaceHolder().pipe(
      map((usersListResponse) => {
        ...
        const hasUser = //verifica se encontrou um usuario no retorno da API de acordo com a PROP-TO-CHECK
          usersListResponse
          .find(
            (user) => user[this.propToCheck].toLocaleLowerCase() === control.value.trim().toLocaleLowerCase()
          ); 
